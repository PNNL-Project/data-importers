...
import xyz.drshu.cs6510.dataProcessor.model.*;
...
public class Parser {
    ...
    private final String MAPPER_NAMESPACE = "xyz.drshu.cs6510.dataProcessor.mapper.";
    ...
    public void parseAttributes(String[] attributes){
        for (int i = 0; i < attributes.length; i++) {
            if(attributes[i].isEmpty() || attributes[i].isBlank()){
                continue;
            }
    ...
    private void insertVav(SqlSession sqlSession) {
        for(Vav vav:vavList){
            sqlSession.insert(MAPPER_NAMESPACE + "VavMapper.insert",vav);
        }
        for (VavThresholds vavThresholds: vavThresholdsList){
            sqlSession.insert(MAPPER_NAMESPACE + "VavThresholdsMapper.insert",vavThresholds);
        }
        sqlSession.commit();
    }
